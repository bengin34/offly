...
export default function MemoryDetailScreen() {
  ...
  const loadData = useCallback(async () => {
    if (!id) return;
    try {
      const memoryData = await MemoryRepository.getWithRelations(id);
      setMemory(memoryData);
      if (memoryData) {
        const chapterData = await ChapterRepository.getById(memoryData.chapterId);
    ...
    Alert.alert(
      ...
      [
        { text: t('common.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await MemoryRepository.deletePhoto(photoId);
              // Update local state
              setMemory((prev) => {
                if (!prev) return null;
                return {
      ...
      {memory && !isLoading ? (
        <View style={styles.container}>
          ...
          <ScrollView style={styles.scroll}>
            ...
            <View style={styles.metaRow}>
              <View style={styles.metaLeft}>
                ...
                >
                  <Ionicons
                    name={memory.memoryType === 'milestone' ? 'flag' : 'document-text'}
                    size={14}
                    color={theme.white}
                  />
            ...
            {/* Location */}
            {(memory.locationName || memory.latitude) && (
              <View style={styles.locationWrapper}>
                <LocationCard
                  locationName={memory.locationName}
                  latitude={memory.latitude}
                  longitude={memory.longitude}
                  mapUrl={memory.mapUrl}
                />
              </View>
            )}
          ...
          {memory.photos.length > 0 && (
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>
                {t('entryDetail.allPhotos', { count: memory.photos.length }).toLocaleUpperCase(locale)}
              </Text>
          ...
          <View style={styles.timestamps}>
            <Text style={styles.timestamp}>
              {t('entryDetail.created', { date: formatDateTime(memory.createdAt) })}
            </Text>
            {memory.updatedAt !== memory.createdAt && (
              <Text style={styles.timestamp}>
                {t('entryDetail.updated', { date: formatDateTime(memory.updatedAt) })}
              </Text>
            )}
          </View>
          </View>
          ...
          <View style={styles.modalContainer}>
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                const photo = memory.photos[currentIndex];
                if (photo) {
            ...
            {memory.photos.length > 1 && (
              <View style={styles.photoCounter}>
                <Text style={styles.photoCounterText}>
                  {(fullScreenPhotoIndex ?? 0) + 1} / {memory.photos.length}
                </Text>
              </View>
            )}
          </View>
      ...
      >
        <Pressable style={styles.actionOverlay} onPress={closeActionMenu}>
          <Pressable
            ...
            onPress={() => {}}
          >
            <TouchableOpacity style={styles.actionItem} onPress={handleEditMemory}>
              <Ionicons name="pencil" size={18} color={theme.text} />
              <Text style={styles.actionText}>{t('navigation.editEntry')}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.actionItem} onPress={handleDeleteMemory}>
